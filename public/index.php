...
foreach ($routes as $route_key => $route_values) {
    // kalo ketemu url routenya
    if(preg_match('#^'.$route_key.'$#', $urls, $params)){
        // hapus parameter pertama
        array_shift($params);

        //url ketemu
        $found = true;
        if(strtoupper($route_values['method']) === strtoupper($_SERVER['REQUEST_METHOD'])){
            // kasi akses
            $notAllowed = false;

            //tentuin engine mana yang dipake
            if(is_string($route_values['engine'])){
                $class = explode('@', $route_values['engine']);

                load($class[0]);
                $controller = new $class[0]();
                call_user_func_array([$controller, $class[1]], $params);
            }else{
                call_user_func_array($route_values['engine'], $params);
            }
        }
